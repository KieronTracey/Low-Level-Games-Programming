...
{
	if (mp_sceneManager->mb_loadedFile)
	{
	...
	{
		mp_UI->PrintData<string>("Scene Not Loaded please correct this before trying again");
	}
}
...
void RenderingEngine::CalculateFramesToRender()
{
	...
	float li_totalFrames = mp_sceneManager->mf_duriation * mp_sceneManager->mi_framesPerSecond;
	int li_framesPerSecond = mp_sceneManager->mi_framesPerSecond;
	...
	for (int i(0); i < li_totalFrames; ++i)
	{
		...
		for (int j(0); j < mp_sceneManager->mp_sceneObjects.size(); ++j)
		{
			//- Making sure that the element is meant to be animated -//
			if (mp_sceneManager->mp_sceneObjects[j]->m_startPosition != mp_sceneManager->mp_sceneObjects[j]->m_endPosition)
			{
				Sphere* temp = new Sphere();
				*temp = *mp_sceneManager->mp_sceneObjects[j];
				...
				temp->m_center.x -= (mp_sceneManager->mp_sceneObjects[j]->m_startPosition.x - mp_sceneManager->mp_sceneObjects[j]->m_endPosition.x) * lf_lerpPercentage;
				temp->m_center.y -= (mp_sceneManager->mp_sceneObjects[j]->m_startPosition.y - mp_sceneManager->mp_sceneObjects[j]->m_endPosition.y) * lf_lerpPercentage;
				temp->m_center.z -= (mp_sceneManager->mp_sceneObjects[j]->m_startPosition.z - mp_sceneManager->mp_sceneObjects[j]->m_endPosition.z) * lf_lerpPercentage;
			...
			{
				l_tempSceneFrameStorage.push_back(mp_sceneManager->mp_sceneObjects[j]);
			}
		}
...
Vec3f RenderingEngine::trace(const Vec3f& rayorig, const Vec3f& raydir, const std::vector<Sphere*>& spheres, const int& depth)
{
	...
	if ((sphere->mf_transparency > 0 || sphere->mf_reflection > 0) && depth < MAX_RAY_DEPTH) {
		...
		surfaceColor = (
			...
			refraction * (1 - fresneleffect) * sphere->mf_transparency) * sphere->m_surfaceColor;
	}
	else {
