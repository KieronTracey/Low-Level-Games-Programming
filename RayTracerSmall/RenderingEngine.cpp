...
void RenderingEngine::CalculateFramesToRender()
{
	...
	for (int i(0); i < li_totalFrames; ++i)
	{
		...
		for (int j(0); j < mp_sceneManager->mp_sceneObjects.size(); ++j)
		{
			...
			if (mp_sceneManager->mp_sceneObjects[j]->OBJ_startpos != mp_sceneManager->mp_sceneObjects[j]->OBJ_endpos)
			{
				...
				*temp = *mp_sceneManager->mp_sceneObjects[j];
				...
				temp->OBJ_center.x -= (mp_sceneManager->mp_sceneObjects[j]->OBJ_startpos.x - mp_sceneManager->mp_sceneObjects[j]->OBJ_endpos.x) * lf_lerpPercentage;
				temp->OBJ_center.y -= (mp_sceneManager->mp_sceneObjects[j]->OBJ_startpos.y - mp_sceneManager->mp_sceneObjects[j]->OBJ_endpos.y) * lf_lerpPercentage;
				temp->OBJ_center.z -= (mp_sceneManager->mp_sceneObjects[j]->OBJ_startpos.z - mp_sceneManager->mp_sceneObjects[j]->OBJ_endpos.z) * lf_lerpPercentage;
